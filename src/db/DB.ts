...
export function DB<EntityModel, EntityRawModel>(
    ...
    @StaticImplements<IDB<EntityModel, EntityRawModel>>()
    class DB {
        ...
        public static getFilteredRaw(filterAttributes: Partial<EntityRawModel> | undefined, arrayContainsAttribute: { arrayName: string, value: string } | undefined): Promise<EntityRawModel[]> {
            const scanParams = DB.buildScanParams(filterAttributes, arrayContainsAttribute);
            return DB.scanRaw(scanParams, []);
        }

        public static async getFiltered(filterAttributes: undefined | object, arrayContainsAttribute: { arrayName: string, value: string } | undefined): Promise<EntityModel[]> {
            const rawEntities = await DB.getFilteredRaw(filterAttributes, arrayContainsAttribute);

            return extend(rawEntities);
        }

        private static async scanRaw(params: DynamoDB.DocumentClient.ScanInput, items: EntityRawModel[]): Promise<EntityRawModel[]> {
        ...
        public static async deleteByIds(ids: string[] | undefined): Promise<string[]> {
        ...
        public static async deleteScanFiltered(filterAttributes: Partial<EntityRawModel> | undefined, arrayContains?: { arrayName: string, value: string } | undefined ): Promise<{}> {
            const scanParams = DB.buildScanParams(filterAttributes, arrayContains);
            return DB.scanDelete(scanParams);
        }

        private static buildScanParams(filterAttributes: Partial<EntityRawModel> | undefined, arrayContainsAttribute?: { arrayName: string, value: string } | undefined): DynamoDB.DocumentClient.ScanInput {
            const tableName = DB.getTableName();
        ...
        private static async scanDelete(params: DynamoDB.DocumentClient.ScanInput): Promise<{}> {
            const partitionKeyName = DB.getPartitionKeyName();
            const sortKeyName = DB.getSortKeyName();
            const sortKeySeparator = DB.getSortKeySeparator();
        ...
        public static async createBatch(entities: EntityRawModel[]): Promise<EntityRawModel[]> {
            const putRequests = [];
        ...
        */
       private static async batchWriteWithRetry(params: DynamoDB.BatchWriteItemInput, retry: number): Promise<{}> {
